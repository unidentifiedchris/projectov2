...
void separarD(l_nodo* p, l_nodo* l , l_nodo **AX,l_nodo **posicion,char dest[100], char Nom[9]){
	l_nodo *ANT = NULL;
	*AX = l ;
	char *ruta, *validacion,*val2,copia[100];
	int contador = 0, fix = 0;
	strcpy (copia,dest);
	ruta = strtok (dest,"/");
		validacion = strtok (copia,"/");
		val2 = strtok (copia,"/");
		if ((strcmp(validacion,".")== 0) && (val2 == NULL)) {
		*posicion = p;
		//*AX
		}else if ((strcmp(validacion,"..")== 0) && (val2 == NULL)) {
		*posicion = p->pPA;
		}else if (strtok (copia,"/")== NULL){}
		else{
	while (ruta != NULL){		
